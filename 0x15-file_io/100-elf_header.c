...
/**
 * print_magic_no - Prints the magic numbers of an ELF header.
 * @e_ident: A pointer to an array containing the ELF magic numbers.
 *
...
/**
 * print_file_data - Prints the data of an ELF header.
 * @e_ident: A pointer to an array containing the ELF class.
 */
...
void print_file_data(unsigned char *e_ident)
{
	...
	case ELFDATANONE:
		...
		break;
	case ELFDATA2LSB:
		printf("2's complement is, little endian\n");
		break;
	case ELFDATA2MSB:
		printf("2's complement is, big endian\n");
		break;
	default:
		printf("<unknown: %x>\n", e_ident[EI_CLASS]);
	}
...
/**
 * main_info - Displays the information contained in the
...
int main_info(int __attribute__((__unused__)) argc, char *argv[])
{
	Elf64_Ehdr *header;
	int o, r;
